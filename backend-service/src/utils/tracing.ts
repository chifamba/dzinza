...
const consoleLogger = {
  info: (message: string, ...args: any[]) => console.log(`[Tracing INFO] ${message}`, ...args),
  error: (message: string, ...args: any[]) => console.error(`[Tracing ERROR] ${message}`, ...args),
};
...
export const initTracer = (serviceName: string, jaegerEndpoint: string, nodeEnv: string) => {
  ...
  const sdk = new NodeSDK({
    ...
    instrumentations: [
      ...
      new MongoDBInstrumentation({ // If backend-service also uses MongoDB directly
