...
import { logger } from '@shared/utils/logger'; // Assuming shared logger
...
try {
  ...
  client.ping()
    ...
    .catch(error => logger.error('Elasticsearch client connection error:', error));
...
export const getElasticsearchClient = (): Client => {
  if (!client) {
    ...
    logger.error('Elasticsearch client is not initialized. Attempting to re-initialize (not recommended for production).');
    ...
    throw new Error('Elasticsearch client is not available. Check logs for initialization errors.');
  }
  return client;
};
