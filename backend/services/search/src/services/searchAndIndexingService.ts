...
export class SearchAndIndexingService {
  ...
  constructor() {
    try {
      ...
      logger.error('Failed to get Elasticsearch client in SearchAndIndexingService:', error);
      throw error;
    }
  }

  async indexDocument({ indexName, documentId, documentBody }: IndexDocumentParams): Promise<void> {
    if (!this.client) {
    ...
    try {
      ...
      logger.info(`Document ${documentId} indexed successfully in ${indexName}.`);
    } catch (error) {
      logger.error(`Error indexing document ${documentId} in ${indexName}:`, error);
  ...
  async deleteDocument(indexName: string, documentId: string): Promise<void> {
    if (!this.client) {
      logger.error('Elasticsearch client not available. Cannot delete document.');
      throw new Error('Elasticsearch client not available.');
    }
    try {
      ...
      logger.info(`Document ${documentId} deleted successfully from ${indexName}.`);
    } catch (error: any) {
      ...
      if (error.meta && error.meta.statusCode === 404) {
        logger.warn(`Document ${documentId} not found in ${indexName} for deletion, but considered successful.`);
        return; // Or handle as desired
      }
      logger.error(`Error deleting document ${documentId} from ${indexName}:`, error);
      throw error;
    }
  }

  async bulkIndexDocuments({ indexName, documents }: BulkIndexDocumentsParams): Promise<void> {
    if (!this.client) {
      logger.error('Elasticsearch client not available. Cannot bulk index documents.');
      throw new Error('Elasticsearch client not available.');
    }
    ...
    const operations = documents.flatMap(doc => [{ index: { _index: indexName, _id: doc.id } }, doc.body]);

    try {
      const bulkResponse = await this.client.bulk({ refresh: 'wait_for', operations });
        ...
        bulkResponse.items.forEach((action: any, i: number) => {
          ...
          if (action[operation].error) {
            erroredDocuments.push({
              ...
              id: documents[Math.floor(i / 2)].id,
              error: action[operation].error,
            });
          }
        });
        logger.error(`Errors during bulk indexing in ${indexName}:`, erroredDocuments);
        throw new Error(`Bulk indexing completed with errors. ${erroredDocuments.length} documents failed.`);
      } else {
        logger.info(`Bulk indexing of ${documents.length} documents to ${indexName} completed successfully.`);
      }
    } catch (error) {
      logger.error(`Error during bulk indexing operation in ${indexName}:`, error);
      throw error;
    }
  }
  ...
  async performSearch(params: SearchParams): Promise<SearchResults> {
    if (!this.client) {
      logger.error('Elasticsearch client not available. Cannot perform search.');
      throw new Error('Elasticsearch client not available.');
    }
    ...
    const typeToDefinitionMap: Record<SearchableType, { index: string; fields: string[]; highlightFields: Record<string,object> }> = {
      person: {
        index: 'persons',
        fields: ["firstName^3", "lastName^3", "fullName^3", "biography", "birthPlaceName", "deathPlaceName"],
        highlightFields: {
      ...
      event: {
        index: 'events',
        fields: ["title^3", "plainTextContent", "placeName", "category^2", "tags^2"],
        highlightFields: {
            "title": {},
    ...
    if (types.length > 0) {
      types.forEach(type => {
        if (typeToDefinitionMap[type]) {
          ...
          highlightFieldsConfig = {...highlightFieldsConfig, ...typeToDefinitionMap[type].highlightFields};
        }
      });
    } else { // Default to all types
      targetIndices = defaultSearchIndices;
      defaultSearchIndices.forEach(indexName => {
        const typeEntry = Object.entries(typeToDefinitionMap).find(([, val]) => val.index === indexName);
        if (typeEntry) {
          const typeDef = typeEntry[1];
          multiMatchFields.push(...typeDef.fields);
          highlightFieldsConfig = {...highlightFieldsConfig, ...typeDef.highlightFields};
        }
      });
    }
    ...
    if (targetIndices.length === 0) {
      logger.warn('Search attempted with no valid types or default indices. Returning empty results.');
      return { data: [], pagination: { total: 0, page, limit, totalPages: 0 } };
    }
    if (multiMatchFields.length === 0) {
        logger.warn('Search attempted with no fields to search on. Returning empty results.');
        return { data: [], pagination: { total: 0, page, limit, totalPages: 0 } };
    }
    ...
    if (sortBy === 'relevance') {
      ...
      sortClause.push({ createdAt: { order: sortOrder, unmapped_type: "date" } });
    } else if (sortBy === 'name') {
      ...
      if (targetIndices.includes('persons')) {
        sortClause.push({ "fullName.keyword": { order: sortOrder, unmapped_type: "keyword", missing: "_last" } });
      }
      if (targetIndices.includes('events')) {
        sortClause.push({ "title.keyword": { order: sortOrder, unmapped_type: "keyword", missing: "_last" } });
      }
      ...
      if(sortClause.length === 0) { // if no specific sort field was added for name
    ...
    try {
      const response = await this.client.search({
        ...
        body: {
            query: esQuery,
            sort: sortClause,
            highlight: Object.keys(highlightFieldsConfig).length > 0 ? {
                pre_tags: ["<mark>"],
                post_tags: ["</mark>"],
                encoder: "html",
      ...
      const mappedResults: SearchResultHit[] = response.hits.hits.map((hit: any) => ({
        _id: hit._id,
        _score: hit._score,
        _index: hit._index,
        _source: hit._source,
      ...
      if ((error as any).meta?.body?.error?.type === 'index_not_found_exception') {
        logger.warn(`Search failed because one or more indices were not found: ${targetIndices.join(',')}. Returning empty results.`);
        return { data: [], pagination: { total: 0, page, limit, totalPages: 0 } };
      }
