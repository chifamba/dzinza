...
const app = express();
const PORT = process.env.GENEALOGY_PORT || 3003;
...
const connectDB = async () => {
  try {
    const mongoUri = process.env.MONGODB_URI || 'mongodb://localhost:27017/dzinza_genealogy';
...
const gracefulShutdown = (signal: string) => {
  logger.info(`Received ${signal}. Graceful shutdown...`, { service: 'genealogy' });
