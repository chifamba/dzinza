...
// It's crucial that the primary authMiddleware attaches the 'roles' array.
export interface AuthenticatedUserWithRoles {
  id: string;
  roles?: string[]; // Assuming roles is an array of strings like ['user', 'admin']
  // Add other properties from req.user if they are consistently available and needed
  name?: string;
  email?: string;
}
 ...
 */
export const adminAuth = (req: RequestWithUser, res: Response, next: NextFunction): void => {
