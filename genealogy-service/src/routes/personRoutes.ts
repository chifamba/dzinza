...
import { duplicateDetectionQueue } from "../queue/duplicateDetectionQueue.js";
...
router.post(
  ...
  [
    ...
    body("gender")
      .optional()
      .isIn(["Male", "Female", "Non-binary", "Other", "Unknown"]),
    ...
    body("identifiers.*.type")
      .if(body("identifiers").exists())
      .isIn([
        "NationalID",
        "Passport",
        "DriverLicense",
    ...
    body("identifiers.*.value")
      .if(body("identifiers").exists())
      .isString()
      .notEmpty()
      .withMessage("Identifier value cannot be empty."),
    body("identifiers.*.verificationStatus")
      .optional()
      .isIn(["Verified", "Unverified", "Pending"]),

    // Basic Parent IDs (ObjectId strings)
    body("biologicalMotherId")
      .optional()
      .isMongoId()
      .withMessage("Invalid biological mother ID format."),
    body("biologicalFatherId")
      .optional()
      .isMongoId()
      .withMessage("Invalid biological father ID format."),

    // Legal Parents (array of objects with parentId and relationshipType)
    body("legalParents").optional().isArray(),
    body("legalParents.*.parentId")
      .if(body("legalParents").exists())
      .isMongoId()
      .withMessage("Invalid legal parent ID format."),
    body("legalParents.*.relationshipType")
      .if(body("legalParents").exists())
      .isIn(["Adoptive", "Guardian", "Foster", "Step-parent", "Other"])
      .withMessage("Invalid legal parent relationship type."),
    ...
    body("traditionalTitles.*")
      .if(body("traditionalTitles").exists())
      .isString()
      .trim(),

    body("privacySettings").optional().isObject(),
    body("privacySettings.showProfile")
      .optional()
      .isIn(["Public", "FamilyTreeOnly", "Private"]),
  ...
  async (req: Request, res: Response) => {
    ...
    if (!userId) {
      logger.warn("Attempt to create person without authenticated user.", {
        treeId,
    ...
    try {
      ...
      if (!familyTree) {
        logger.warn("FamilyTree not found for person creation.", {
          userId,
          treeId,
          correlationId: req.correlationId,
        });
      ...
      if (!familyTree.canUserEdit(userId)) {
        logger.warn("User lacks permission to add person to family tree.", {
          userId,
          treeId,
          correlationId: req.correlationId,
        });
        return res.status(403).json({
          error:
            "Access denied: You do not have permission to add persons to this tree.",
        });
      }
      ...
      if (personData.biologicalMotherId) {
        const mother = await Person.findOne({
          _id: personData.biologicalMotherId,
          familyTreeId: treeId,
        });
        if (!mother) {
          return res.status(400).json({
            error: `Biological mother with ID ${personData.biologicalMotherId} not found in this tree.`,
          });
        }
      }
      if (personData.biologicalFatherId) {
        const father = await Person.findOne({
          _id: personData.biologicalFatherId,
          familyTreeId: treeId,
        });
        if (!father) {
          return res.status(400).json({
            error: `Biological father with ID ${personData.biologicalFatherId} not found in this tree.`,
          });
        }
      }
      ...
      if (personData.legalParents && personData.legalParents.length > 0) {
        for (const legalParent of personData.legalParents) {
          const parentDoc = await Person.findOne({
            _id: legalParent.parentId,
            familyTreeId: treeId,
          });
          if (!parentDoc) {
            return res.status(400).json({
              error: `Legal parent with ID ${legalParent.parentId} not found in this tree.`,
            });
          }
        }
      }
      ...
      // Enqueue a duplicate check job
      await duplicateDetectionQueue.add("check-duplicate", {
      ...
      familyTree.statistics.totalPersons =
        (familyTree.statistics.totalPersons || 0) + 1;
      await familyTree.save();

      logger.info("New person created in family tree.", {
        userId,
        treeId,
      ...
      if (
        ...
        err.message.includes("uniqueId")
      ) {
        return res
          .status(400)
          .json({ error: "A person with this uniqueId already exists." });
      }
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!userId) {
      logger.warn("Attempt to get person without authenticated user.", {
        treeId,
        personId,
        correlationId: req.correlationId,
      });
    ...
    try {
      ...
      if (!familyTree) {
        logger.warn("FamilyTree not found for person retrieval.", {
          userId,
          treeId,
          personId,
          correlationId: req.correlationId,
      ...
      if (!familyTree.canUserView(userId)) {
        logger.warn(
          "User lacks permission to view family tree for person retrieval.",
          { userId, treeId, personId, correlationId: req.correlationId }
        );
        return res.status(403).json({
          error:
      ...
      const person = await Person.findOne({
        ...
        .populate(
          "biologicalMother",
          "firstName lastName nickName profilePhotoUrl dateOfBirth dateOfDeath isLiving gender uniqueId"
        )
        .populate(
          "biologicalFather",
          "firstName lastName nickName profilePhotoUrl dateOfBirth dateOfDeath isLiving gender uniqueId"
      ...
      if (!person) {
        logger.warn("Person not found in specified family tree.", {
          userId,
          treeId,
          personId,
          correlationId: req.correlationId,
...
router.put(
  ...
  [
    ...
    body("spouses").custom((value) => {
      if (value !== undefined) {
        throw new Error(
          "Spouses array cannot be updated directly through this endpoint."
        );
      }
      return true;
    }),
    body("siblings").custom((value) => {
      if (value !== undefined) {
        throw new Error(
          "Siblings array cannot be updated directly through this endpoint."
        );
      }
      return true;
    }),
  ...
  async (req: Request, res: Response) => {
    ...
    if (!userId) {
      logger.warn("Attempt to update person without authenticated user.", {
        treeId,
        personId,
        correlationId: req.correlationId,
      });
    ...
    try {
      ...
      if (!familyTree) {
        logger.warn("FamilyTree not found for person update.", {
          userId,
          treeId,
          personId,
          correlationId: req.correlationId,
      ...
      if (!familyTree.canUserEdit(userId)) {
        logger.warn(
          "User lacks permission to edit family tree for person update.",
          { userId, treeId, personId, correlationId: req.correlationId }
        );
        return res.status(403).json({
          error:
            "Access denied: You do not have permission to edit persons in this tree.",
        });
      }
      ...
      if (!person) {
        logger.warn("Person not found for update in specified family tree.", {
          userId,
          treeId,
          personId,
          correlationId: req.correlationId,
      ...
      if (updates.biologicalMotherId) {
        const mother = await Person.findOne({
          _id: updates.biologicalMotherId,
          familyTreeId: treeId,
        });
        if (!mother)
          return res.status(400).json({
            error: `Biological mother with ID ${updates.biologicalMotherId} not found in this tree.`,
          });
      } else if (updates.biologicalMotherId === null) {
      ...
      if (updates.biologicalFatherId) {
        const father = await Person.findOne({
          _id: updates.biologicalFatherId,
          familyTreeId: treeId,
        });
        if (!father)
          return res.status(400).json({
            error: `Biological father with ID ${updates.biologicalFatherId} not found in this tree.`,
          });
      ...
      if (updates.legalParents && Array.isArray(updates.legalParents)) {
        for (const legalParent of updates.legalParents) {
          if (legalParent.parentId) {
            ...
            return res
              .status(400)
              .json({ error: `Legal parent entry missing parentId.` });
          }
        }
      ...
      if (updates.privacySettings) {
        if (person.privacySettings) {
          // Merge if already exists
          for (const key in updates.privacySettings) {
            if (
              Object.prototype.hasOwnProperty.call(updates.privacySettings, key)
            ) {
              (person.privacySettings as any)[key] =
                updates.privacySettings[key];
            }
          }
        } else {
      ...
      if (
        Object.prototype.hasOwnProperty.call(updates, "biologicalMotherId") &&
        updates.biologicalMotherId !== null
      ) {
        person.biologicalMother = updates.biologicalMotherId;
      }
      if (
        Object.prototype.hasOwnProperty.call(updates, "biologicalFatherId") &&
        updates.biologicalFatherId !== null
      ) {
      ...
      await Person.populate(updatedPerson, [
        {
          path: "biologicalMother",
          select: "firstName lastName nickName profilePhotoUrl uniqueId",
        },
        {
          path: "biologicalFather",
          select: "firstName lastName nickName profilePhotoUrl uniqueId",
        },
        {
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!userId) {
      logger.warn("Attempt to delete person without authenticated user.", {
        treeId,
        personId,
        correlationId: req.correlationId,
      });
    ...
    try {
      ...
      if (!familyTree) {
        logger.warn("FamilyTree not found for person deletion.", {
          userId,
          treeId,
          personId,
          correlationId: req.correlationId,
      ...
      if (!familyTree.canUserEdit(userId)) {
        // Or a more specific canUserDelete if available
        logger.warn(
          "User lacks permission to delete person from family tree.",
          { userId, treeId, personId, correlationId: req.correlationId }
        );
        return res.status(403).json({
          error:
            "Access denied: You do not have permission to delete persons from this tree.",
        });
      }
      ...
      if (!personToDelete) {
        logger.warn("Person not found for deletion in specified family tree.", {
          userId,
          treeId,
          personId,
          correlationId: req.correlationId,
      ...
      logger.info(
        `Deleted relationships involving person ${personId} from tree ${treeId}`,
        { personId, treeId, correlationId: req.correlationId }
      );
