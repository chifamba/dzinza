...
const SEARCH_SERVICE_URL =
  process.env.SEARCH_SERVICE_URL || "http://localhost:3003/api"; // Search service runs on port 3003
...
router.post(
  ...
  [
    ...
    body("relatedPersons")
      .optional()
      .isArray()
      .withMessage("relatedPersons must be an array"),
    body("relatedPersons.*").optional({ checkFalsy: true }).isString().trim(),
    body("associatedMediaIds")
      .optional()
      .isArray()
      .withMessage("associatedMediaIds must be an array"),
    body("associatedMediaIds.*")
  ...
  async (req: Request, res: Response) => {
    ...
    if (!req.user?.id) {
      return res
        .status(401)
        .json({ message: "User not authenticated correctly" });
    }
    ...
    try {
      ...
      if (
        newEvent.associatedMediaIds &&
        newEvent.associatedMediaIds.length > 0
      ) {
        const STORAGE_SERVICE_URL =
          process.env.STORAGE_SERVICE_URL || "http://storage-service:80"; // Placeholder

        for (const mediaId of newEvent.associatedMediaIds) {
          try {
            const response = await fetch(
              `${STORAGE_SERVICE_URL}/api/files/${mediaId}/associate-event`,
              {
                method: "PUT",
                headers: {
            ...
            if (!response.ok) {
              ...
              logger.warn(
                `Failed to associate media ${mediaId} with event ${newEvent._id}. Status: ${response.status}. Body: ${errorBody}`
              );
            } else {
              logger.info(
                `Successfully associated media ${mediaId} with event ${newEvent._id}`
              );
            }
          } catch (fetchError) {
            logger.error(
              `Error calling storage service to associate media ${mediaId} with event ${newEvent._id}:`,
              fetchError
            );
          }
        }
      ...
      recordActivity({
        ...
        familyTreeId: newEvent.familyTreeId
          ? newEvent.familyTreeId.toString()
          : undefined,
        targetResourceId: newEvent._id.toString(),
        targetResourceType: "Event",
        targetResourceName: newEvent.title,
        details: `${req.user.name || req.user.id} created event '${
          newEvent.title
        }'.`,
        ipAddress: req.ip,
      ...
      fetch(`${SEARCH_SERVICE_URL}/index`, {
        ...
        .then(async (searchRes) => {
          if (!searchRes.ok) {
            ...
            logger.warn(
              `Failed to index new event ${newEvent._id}. Status: ${searchRes.status}. Body: ${errorBody}`
            );
          } else {
            logger.info(
              `Event ${newEvent._id} submitted for indexing successfully.`
            );
          }
        })
        .catch((searchErr) => {
          logger.error(
            `Error calling search service to index new event ${newEvent._id}:`,
            searchErr
          );
        });
    ...
    try {
      ...
      Object.keys(updates).forEach((key) => {
        if (
          ...
          // Allow unsetting optional fields by passing null
          if (
            ["date", "endDate", "familyTreeId", "category", "place"].includes(
              key
            )
          ) {
      ...
      else if (updates.associatedMediaIds !== undefined)
        event.associatedMediaIds = updates.associatedMediaIds;
      ...
      const removedMediaIds = originalMediaIds.filter(
        (id) => !newMediaIds.includes(id)
      );

      const STORAGE_SERVICE_URL =
        process.env.STORAGE_SERVICE_URL || "http://storage-service:80";
        ...
        try {
          const response = await fetch(
            `${STORAGE_SERVICE_URL}/api/files/${mediaId}/associate-event`,
            {
              ...
              headers: {
                "Content-Type":
                  "application/json" /* TODO: Service-to-service auth */,
              },
          ...
          if (!response.ok) {
            logger.warn(
              `Failed to associate new media ${mediaId} with event ${eventIdStr}. Status: ${response.status}`
            );
          } else {
            logger.info(
              `Successfully associated new media ${mediaId} with event ${eventIdStr}`
            );
          }
        } catch (fetchError) {
          logger.error(
            `Error calling storage service for new media association (event ${eventIdStr}, media ${mediaId}):`,
            fetchError
          );
        }
      }
      ...
      for (const mediaId of removedMediaIds) {
        try {
          const response = await fetch(
            `${STORAGE_SERVICE_URL}/api/files/${mediaId}/disassociate-event`,
            {
              method: "PUT",
              headers: {
                "Content-Type":
          ...
          if (!response.ok) {
            logger.warn(
              `Failed to disassociate old media ${mediaId} from event ${eventIdStr}. Status: ${response.status}`
            );
          } else {
            logger.info(
              `Successfully disassociated old media ${mediaId} from event ${eventIdStr}`
            );
          }
        } catch (fetchError) {
          logger.error(
            `Error calling storage service for old media disassociation (event ${eventIdStr}, media ${mediaId}):`,
            fetchError
          );
        }
      }
      ...
      fetch(`${SEARCH_SERVICE_URL}/index`, {
        ...
        .then(async (searchRes) => {
          if (!searchRes.ok) {
            const errorBody = await searchRes.text();
            logger.warn(
              `Failed to update index for event ${updatedEvent._id}. Status: ${searchRes.status}. Body: ${errorBody}`
            );
          } else {
            logger.info(
              `Event ${updatedEvent._id} submitted for re-indexing successfully.`
            );
          }
        })
        .catch((searchErr) => {
          logger.error(
            `Error calling search service to update index for event ${updatedEvent._id}:`,
            searchErr
          );
        });
        ...
        for (const mediaId of event.associatedMediaIds) {
          try {
            ...
            if (!response.ok) {
              const errorBody = await response.text();
              logger.warn(
                `Failed to disassociate media ${mediaId} from event ${eventIdStr} during event deletion. Status: ${response.status}. Body: ${errorBody}`
              );
            } else {
              logger.info(
                `Successfully disassociated media ${mediaId} from event ${eventIdStr} during event deletion.`
              );
            }
          } catch (fetchError) {
            logger.error(
              `Error calling storage service to disassociate media ${mediaId} from event ${eventIdStr} during event deletion:`,
              fetchError
            );
          }
        }
      ...
      fetch(`${SEARCH_SERVICE_URL}/index/event/${eventIdToDelete}`, {
        ...
        .then(async (searchRes) => {
          if (!searchRes.ok && searchRes.status !== 404) {
            // 404 (not found) is acceptable for delete
            const errorBody = await searchRes.text();
            logger.warn(
              `Failed to delete event ${eventIdToDelete} from index. Status: ${searchRes.status}. Body: ${errorBody}`
            );
          } else {
            logger.info(
              `Event ${eventIdToDelete} submitted for deletion from index successfully (or was not found).`
            );
          }
        })
        .catch((searchErr) => {
          logger.error(
            `Error calling search service to delete event ${eventIdToDelete} from index:`,
            searchErr
          );
        });
    ...
    try {
      ...
      if (!queryUserId && familyTreeId) {
        ...
        queryFilter.$or = [
          { userId: req.user.id }, // Their own events in that tree
          {
            ...
            privacy: { $in: ["public", "family"] },
          }, // Public/Family events in that tree
        ];
        if (queryFilter.userId && queryFilter.familyTreeId) {
          // Avoid redundant userId if already specific
          delete queryFilter.userId; // The $or handles user-specific and tree-specific
        }
      } else if (!queryUserId && !familyTreeId) {
        // General query, not specific tree or user
        // Show user's own events OR public events.
        queryFilter.$or = [{ userId: req.user.id }, { privacy: "public" }];
      }
        ...
        eventsData.forEach((event) => {
          if (event.relatedPersons && event.relatedPersons.length > 0) {
            event.relatedPersons.forEach((personId) => {
              ...
              if (
                personId &&
                mongoose.Types.ObjectId.isValid(personId.toString())
              ) {
      ...
      if (personIdsToFetch.length > 0) {
        try {
          ...
          const personsFromDb = await Person.find({
            _id: { $in: personIdsToFetch },
          })
      ...
      const processedEvents = eventsData.map((event) => {
        ...
        if (event.relatedPersons && event.relatedPersons.length > 0) {
          populatedRelatedPersons = event.relatedPersons
            ...
            .filter((person) => person !== undefined) as {
            id: string;
            name: string;
          }[];
        }
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (isOwner || isPublic || isFamilyViewable) {
        ...
        return res
          .status(403)
          .json({ message: "Access forbidden to this event" });
      }
    } catch (error) {
      logger.error(`Error fetching event ${id}:`, error);
      if (error instanceof mongoose.Error.CastError) {
        // Mongoose specific error for bad ID format before param validation
        return res.status(400).json({ message: "Invalid event ID format" });
      }
