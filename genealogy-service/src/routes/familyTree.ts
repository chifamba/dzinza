...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const query: FilterQuery<FamilyTreeDocument> = {
        // Typed query more specifically
        $or: [
          { ownerId: userId },
          {
            "collaborators.userId": userId,
            "collaborators.acceptedAt": { $exists: true },
          },
        ],
      };
      ...
      logger.error("Error retrieving family trees:", error, {
        userId: req.user?.id,
      });
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      logger.error("Error creating family tree:", error, {
        userId: req.user?.id,
      });
    ...
    await tracer.startActiveSpan(
      ...
      async (span: Span) => {
        try {
          ...
          const familyTree = await tracer.startActiveSpan(
            "familyTree.findById.db",
            async (dbSpan: Span) => {
              try {
                ...
                const tree = await FamilyTree.findById(familyTreeId).populate(
                  "rootPersonId",
          ...
          span.end();
          logger.error("Error retrieving family tree:", err, {
            userId: req.user?.id,
          });
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (updates.name && familyTree.name !== updates.name)
        changedFields.push(`name to "${updates.name}"`); // This comparison is flawed as familyTree.name is already updated.
      ...
      if (updates.privacy && oldPrivacy !== updates.privacy)
        changedFields.push(`privacy to "${updates.privacy}"`);
      if (updates.settings) {
        // Deeper comparison for settings if necessary
        if (
          oldSettings?.allowCollaboration !==
          updates.settings.allowCollaboration
        ) {
          changedFields.push(
            `collaboration to ${
              updates.settings.allowCollaboration ? "enabled" : "disabled"
            }`
          );
        }
      ...
      logger.error("Error updating family tree:", error, {
        userId: req.user?.id,
      });
  ...
  async (req, res) => {
    try {
      ...
      if (familyTree.ownerId !== userId) {
        return res
          .status(403)
          .json({ error: "Only the owner can delete a family tree" });
      }
      ...
      logger.error("Error deleting family tree:", error, {
        userId: req.user?.id,
      });
 ...
 */
router.put(
  "/:treeId/collaborators/:collaboratorUserId",
  [
    param("treeId").isMongoId().withMessage("Invalid family tree ID"),
    param("collaboratorUserId")
      .isString()
      .notEmpty()
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        familyTree.collaborators.find((c) => c.userId === currentUserId)
          ?.role === "admin" &&
      ...
      if (collaboratorIndex === -1) {
        return res
          .status(404)
          .json({ error: "Collaborator not found on this tree." });
      }
      ...
      logger.error("Error updating collaborator role:", error, {
        ...
        collaboratorUserId: req.params.collaboratorUserId,
      });
  ...
  async (req, res) => {
    try {
      ...
      if (familyTree.ownerId === collaboratorUserId) {
        return res
          .status(403)
          .json({ error: "Cannot remove the owner of the tree." });
      }
      ...
      if (updateResult.modifiedCount === 0) {
        ...
        logger.warn(
          "Collaborator removal did not modify the document, might have been already removed or user ID mismatch.",
          { treeId, collaboratorUserId }
        );
  ...
  async (req, res) => {
    try {
      ...
      const statistics = {
        ...
        photoCompleteness:
          totalPersons > 0
            ? Math.round((personsWithPhotos / totalPersons) * 100)
            : 0,
        birthDateCompleteness:
          totalPersons > 0
            ? Math.round((personsWithBirthDates / totalPersons) * 100)
            : 0,
      ...
      logger.error("Error retrieving family tree statistics:", error, {
        userId: req.user?.id,
      });
 ...
 */
router.get(
  "/:treeId/members/:memberId/details",
  [
    param("treeId").isMongoId().withMessage("Invalid family tree ID"),
  ...
  async (req, res) => {
    try {
      ...
      logger.info("Fetching person details with relationships", {
        currentUserId,
        treeId,
        memberId,
        correlationId: req.correlationId,
      ...
      const familyTree = await FamilyTree.findById(treeId);
      if (!familyTree) {
        logger.warn("Family tree not found for details endpoint", {
          currentUserId,
          treeId,
          memberId,
          correlationId: req.correlationId,
      ...
      if (!familyTree.canUserView(currentUserId)) {
        logger.warn("User access denied for family tree details", {
          currentUserId,
          treeId,
          memberId,
          correlationId: req.correlationId,
        ...
        return res
          .status(403)
          .json({ error: "Access denied to this family tree" });
      }

      const personPopulateFields =
        "firstName lastName nickName profilePhotoUrl gender uniqueId dateOfBirth isLiving dateOfDeath";
      ...
      if (!person) {
        logger.warn("Person not found for details endpoint", {
          currentUserId,
          treeId,
          memberId,
          correlationId: req.correlationId,
        ...
        return res
          .status(404)
          .json({ error: "Person not found in this family tree" });
      }

      // Construct parents array
      const parentsResponse = [];
      if (person.biologicalMother) {
        parentsResponse.push({
          personDetails: person.biologicalMother,
          relationshipType: "BiologicalMother",
        });
      }
      if (person.biologicalFather) {
        parentsResponse.push({
          personDetails: person.biologicalFather,
          relationshipType: "BiologicalFather",
        });
      }
      if (person.legalParents) {
        person.legalParents.forEach((lp) => {
          if (lp.parentId) {
            // parentId should be populated here
            parentsResponse.push({
              personDetails: lp.parentId,
              relationshipType: lp.relationshipType,
            });
          }
        });
      }
      ...
      const childrenRelationships = await Relationship.find({
        ...
        type: {
          $in: [RelationshipType.ParentChild, RelationshipType.GuardianChild],
        },
      })
      ...
      logger.error("Error fetching person details with relationships:", {
        userId: req.user?.id,
        treeId,
        memberId,
        error: err.message,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const familyTree = await FamilyTree.findById(treeId);
      if (!familyTree) {
        logger.warn("Family tree not found for GEDCOM import", {
          userId,
          treeId,
          correlationId: req.correlationId,
        });
      ...
      if (!familyTree.canUserEdit(userId)) {
        logger.warn("User access denied for GEDCOM import", {
          userId,
          treeId,
          correlationId: req.correlationId,
        });
        return res
          .status(403)
          .json({ error: "Access denied to edit this family tree" });
      }
      ...
      const gedcomDateStringToDateObject = (
        dateStr: string | undefined
      ): { date?: Date; isEstimate?: boolean } => {
        if (!dateStr) return {};
        let isEstimate = false;
        let cleanDateStr = dateStr;

        if (
          dateStr.startsWith("ABT ") ||
          dateStr.startsWith("EST ") ||
          dateStr.startsWith("CAL ")
        ) {
          isEstimate = true;
      ...
      for (const indiNode of indiNodes) {
        ...
        if (nameNode?.data) {
          ...
          personData.firstName =
            getNodeData(findNodeByTag(nameNode.tree, "GIVN")) ||
            nameParts[0]?.trim();
          personData.lastName =
            getNodeData(findNodeByTag(nameNode.tree, "SURN")) ||
            (nameParts[1] || "").trim() ||
            undefined;
          if (!personData.firstName && personData.lastName) {
            // Handle cases like "/Surname/"
          ...
          personData.nickName = getNodeData(
            findNodeByTag(nameNode.tree, "NICK")
          );
        ...
        if (birtNode) {
          const { date: birthD, isEstimate: birthEstimate } =
            gedcomDateStringToDateObject(
              getNodeData(findNodeByTag(birtNode.tree, "DATE"))
            );
          personData.dateOfBirth = birthD;
          personData.isBirthDateEstimated = birthEstimate;
          personData.placeOfBirth = getNodeData(
            findNodeByTag(birtNode.tree, "PLAC")
          );
        }

        const deatNode = findNodeByTag(indiNode.tree, "DEAT");
        if (deatNode) {
          const { date: deathD, isEstimate: deathEstimate } =
            gedcomDateStringToDateObject(
              getNodeData(findNodeByTag(deatNode.tree, "DATE"))
            );
          personData.dateOfDeath = deathD;
          personData.isDeathDateEstimated = deathEstimate;
          personData.placeOfDeath = getNodeData(
            findNodeByTag(deatNode.tree, "PLAC")
          );
          personData.causeOfDeath = getNodeData(
            findNodeByTag(deatNode.tree, "CAUS")
          );
        ...
        personData.notes =
          findNodesByTag(indiNode.tree, "NOTE")
            .map((n) => n.data)
            .join("\n") || undefined;
        ...
        try {
          ...
          gedcomIndiIdToMongoPersonIdMap.set(
            indiNode.pointer,
          ...
          findNodesByTag(indiNode.tree, "FAMC").forEach((famcNode) => {
            if (famcNode.data) {
              famcLinks.push({
                childMongoId: newPerson._id.toString(),
                famcId: famcNode.data,
              });
            }
          });
          findNodesByTag(indiNode.tree, "FAMS").forEach((famsNode) => {
            if (famsNode.data) {
              famsLinks.push({
                indiMongoId: newPerson._id.toString(),
                famsId: famsNode.data,
              });
            }
          });
        } catch (personSaveError) {
          logger.error("Failed to save person from GEDCOM:", {
            gedcomId: indiNode.pointer,
            name: personData.firstName,
            error: personSaveError,
            correlationId: req.correlationId,
          });
        }
      }
      ...
      for (const famNode of famNodes) {
        ...
        const husbandMongoId = husbNode?.data
          ? gedcomIndiIdToMongoPersonIdMap.get(husbNode.data)
          : undefined;
        const wifeMongoId = wifeNode?.data
          ? gedcomIndiIdToMongoPersonIdMap.get(wifeNode.data)
          : undefined;
        ...
        marriageEventNodes.forEach((marrNode) => {
          const eventDateDetails = gedcomDateStringToDateObject(
            getNodeData(findNodeByTag(marrNode.tree, "DATE"))
          );
          const event = {
            ...
            description:
              findNodesByTag(marrNode.tree, "NOTE")
                .map((n) => n.data)
                .join("\n") || undefined,
          };
        ...
        divorceEventNodes.forEach((divNode) => {
          const eventDateDetails = gedcomDateStringToDateObject(
            getNodeData(findNodeByTag(divNode.tree, "DATE"))
          );
          const event = {
            eventType: "Divorce",
            date: eventDateDetails.date,
            ...
            description:
              findNodesByTag(divNode.tree, "NOTE")
                .map((n) => n.data)
                .join("\n") || undefined,
          };
          relationshipEvents.push(event);
        ...
        if (husbandMongoId && wifeMongoId) {
          try {
            ...
            if (husbandDoc && wifeDoc) {
              const spousalStatus =
                divorceEventNodes.length > 0
                  ? SpousalStatus.Divorced
                  : marriageEventNodes.length > 0
              ...
              relationshipsCreated++;
              famIdToSpousalRelationshipIdMap.set(
                famNode.pointer,
                spouseRel._id.toString()
              );

              // Denormalize: Update spouses array on Person documents
              const spouseLinkHusband = {
                _id: spouseRel._id,
                spouseId: wifeMongoId,
                relationshipType: spousalStatus,
                startDate: spouseRel.startDate,
                endDate: spouseRel.endDate,
              };
              const spouseLinkWife = {
                _id: spouseRel._id,
                spouseId: husbandMongoId,
                relationshipType: spousalStatus,
                startDate: spouseRel.startDate,
                endDate: spouseRel.endDate,
              };
            ...
            logger.error("Failed to save spouse relationship from GEDCOM:", {
              husbandId,
              wifeId,
              error: relSaveError,
              correlationId: req.correlationId,
      ...
      for (const link of famcLinks) {
        ...
        const husbNode = findNodeByTag(famNode.tree, "HUSB");
        const wifeNode = findNodeByTag(famNode.tree, "WIFE");
        const fatherMongoId = husbNode?.data
          ? gedcomIndiIdToMongoPersonIdMap.get(husbNode.data)
          : undefined;
        const motherMongoId = wifeNode?.data
          ? gedcomIndiIdToMongoPersonIdMap.get(wifeNode.data)
          : undefined;
        ...
        if (fatherMongoId) {
          fatherRelExists = await Relationship.exists({
            familyTreeId: treeId,
            person1Id: fatherMongoId,
            person2Id: childDoc._id,
            type: RelationshipType.ParentChild,
          });
        }
        if (motherMongoId) {
          motherRelExists = await Relationship.exists({
            familyTreeId: treeId,
            person1Id: motherMongoId,
            person2Id: childDoc._id,
            type: RelationshipType.ParentChild,
          });
        }
        ...
        const adoptionNode = findNodeByTag(
          findNodeByTag(famNode.tree, "CHIL", link.childMongoId)?.tree,
          "ADOP"
        ); // CHIL pointer is not standard, this check is weak.
        ...
        if (adoptionNode && !fatherIsAdoptive && !motherIsAdoptive) {
          // General ADOP tag for child in this family
        ...
        if (fatherMongoId && !fatherRelExists) {
          const fatherRole = fatherIsAdoptive
            ? ParentalRole.AdoptiveFather
            : ParentalRole.BiologicalFather;
          try {
            const rel = new Relationship({
              ...
              person2Id: childDoc._id,
              type: RelationshipType.ParentChild,
              parentalRole: fatherRole,
            });
            await rel.save();
            relationshipsCreated++;
            if (!fatherIsAdoptive)
              childDoc.biologicalFather = fatherMongoId as any;
            else {
              const existingLegal = childDoc.legalParents.find((lp) =>
                lp.parentId.equals(fatherMongoId)
              );
              if (!existingLegal)
                childDoc.legalParents.push({
                  parentId: fatherMongoId as any,
                  relationshipType: "Adoptive",
                });
            }
          } catch (e) {
            logger.error("Error creating father-child rel", e);
          }
        }
        if (motherMongoId && !motherRelExists) {
          const motherRole = motherIsAdoptive
            ? ParentalRole.AdoptiveMother
            : ParentalRole.BiologicalMother;
          try {
            const rel = new Relationship({
              familyTreeId: treeId,
              person1Id: motherMongoId,
              person2Id: childDoc._id,
              type: RelationshipType.ParentChild,
              parentalRole: motherRole,
            });
            await rel.save();
            relationshipsCreated++;
            if (!motherIsAdoptive)
              childDoc.biologicalMother = motherMongoId as any;
            else {
              const existingLegal = childDoc.legalParents.find((lp) =>
                lp.parentId.equals(motherMongoId)
              );
              if (!existingLegal)
                childDoc.legalParents.push({
                  parentId: motherMongoId as any,
                  relationshipType: "Adoptive",
                });
            }
          } catch (e) {
            logger.error("Error creating mother-child rel", e);
          }
        }
        await childDoc.save();
      }
      ...
      familyTree.statistics.totalPersons =
        (familyTree.statistics.totalPersons || 0) + individualsImported;
      // totalGenerations and completenessScore would need more complex calculation
      await familyTree.save();

      logger.info("GEDCOM import completed with enhanced parsing", {
        userId,
        treeId,
        individualsImported,
        familiesProcessed,
      ...
      logger.error("Error during GEDCOM import endpoint:", error, {
        userId: req.user?.id,
        treeId: req.params.treeId,
        correlationId: req.correlationId,
      });
...
const formatGedcomDateWithEstimate = (
  ...
  const month = date
    ...
    .toUpperCase(); // Ensures uppercase month e.g. JAN
  const year = date.getFullYear();

  let prefix = "";
  if (isEstimate) {
    // Assuming isEstimate is true for ABT, EST, CAL
 ...
 */
router.get(
  "/:treeId/export/gedcom",
  [param("treeId").isMongoId().withMessage("Invalid family tree ID")],
  async (req: Request, res: Response) => {
    const tracer = trace.getTracer("genealogy-service-familytree-routes");
    await tracer.startActiveSpan(
      "familyTree.exportGedcom.handler",
      async (span: Span) => {
        try {
          const errors = validationResult(req);
          if (!errors.isEmpty()) {
          ...
          if (!familyTree) {
            span.setStatus({
              code: SpanStatusCode.ERROR,
              message: "Family tree not found for export",
            });
            span.end();
            return res.status(404).json({ error: "Family tree not found" });
          }
          ...
          if (!familyTree.canUserView(userId)) {
            span.setStatus({
              code: SpanStatusCode.ERROR,
              message: "Access denied for export",
            });
            span.end();
            return res
              .status(403)
          ...
          for (const person of persons) {
            ...
            mongoPersonIdToGedcomIndiId.set(
              person._id.toString(),
              gedcomIndiId
            );
            ...
            if (person.gender) {
              const sex = person.gender.toUpperCase().startsWith("M")
                ? "M"
                : person.gender.toUpperCase().startsWith("F")
                ? "F"
                : "U";
              addLine(1, "SEX", sex);
            }
              ...
              addLine(1, "BIRT", undefined, () => {
                if (person.dateOfBirth)
                  addLine(
                    2,
                    "DATE",
                    formatGedcomDateWithEstimate(
                      new Date(person.dateOfBirth),
                      person.isBirthDateEstimated
                    )
                  );
                if (person.placeOfBirth)
                  addLine(2, "PLAC", person.placeOfBirth);
              });
            }
            if (
              person.dateOfDeath ||
                ...
                if (person.dateOfDeath)
                  addLine(
                    ...
                    formatGedcomDateWithEstimate(
                      ...
                      person.isDeathDateEstimated
                    )
                  );
                if (person.placeOfDeath)
                  addLine(2, "PLAC", person.placeOfDeath);
                if (person.causeOfDeath)
                  addLine(2, "CAUS", person.causeOfDeath);
              });
            }
            ...
            person.identifiers?.forEach((id) => {
              ...
              if (id.type === "Email") addLine(1, "EMAIL", id.value);
                ...
                addLine(1, `_IDNO ${id.value}`, undefined, () => {
                  // Custom tag for other ID numbers
                  if (id.type) addLine(2, "TYPE", id.type);
                  if (id.verificationStatus)
                    addLine(2, "STAT", id.verificationStatus); // Custom for status
            ...
            person.traditionalTitles?.forEach((title) =>
              addLine(1, "TITL", title)
            ); // TITL is somewhat standard for titles/offices
          ...
          const personGedcomMapWithFamLinks: Record<
            string,
            { famc: string[]; fams: string[] }
          > = {};
          persons.forEach(
            (p) =>
              (personGedcomMapWithFamLinks[
                mongoPersonIdToGedcomIndiId.get(p._id.toString())!
              ] = { famc: [], fams: [] })
          );

          for (const rel of relationships) {
            const p1GedId = mongoPersonIdToGedcomIndiId.get(
              rel.person1Id.toString()
            );
            const p2GedId = mongoPersonIdToGedcomIndiId.get(
              rel.person2Id.toString()
            );
            ...
            if (rel.type === RelationshipType.Spousal) {
              ...
              const p1Doc = persons.find(
                (p) => p._id.toString() === rel.person1Id.toString()
              );
              const p2Doc = persons.find(
                (p) => p._id.toString() === rel.person2Id.toString()
              );

              if (
                p1Doc?.gender === "Male" ||
                (p1Doc?.gender !== "Female" && p2Doc?.gender === "Female")
              ) {
                  ...
                  addLine(1, "MARR", undefined, () => {
                    if (event.date)
                      addLine(
                        ...
                        formatGedcomDateWithEstimate(
                          new Date(event.date),
                          (event as any).isDateEstimated
                        )
                      ); // Assuming isDateEstimated on event
                    if (event.place) addLine(2, "PLAC", event.place);
                    if (event.description)
                      addLine(2, "NOTE", event.description);
                  });
              ...
              if (
                rel.status &&
                rel.status !== SpousalStatus.Married &&
                rel.status !== SpousalStatus.Divorced
              ) {
                addLine(1, "NOTE", `Relationship Status: ${rel.status}`);
              }

              // Add children to this spousal FAM record
              const childrenInThisFamily = relationships
                .filter(
                  (rChild) =>
                    rChild.type === RelationshipType.ParentChild &&
                    ((rChild.person1Id.equals(rel.person1Id) &&
                      rChild.person2Id.equals(rel.person2Id) === false) || // p1 is parent, p2 is child
                      (rChild.person1Id.equals(rel.person2Id) &&
                        rChild.person2Id.equals(rel.person1Id) === false)) && // p2 is parent, p1 is child
                    ...
                    relationships.some(
                      (rOtherParent) =>
                        ...
                        (rOtherParent.person1Id.equals(rel.person1Id) ||
                          rOtherParent.person1Id.equals(rel.person2Id)) // other parent is one of the spouses
                    )
                )
                .map((rChild) =>
                  mongoPersonIdToGedcomIndiId.get(rChild.person2Id.toString())
                )
                .filter(Boolean);

              // Simplified: Find children where one parent is p1 and other parent is p2 of spousal rel
              const childrenOfThisCouple = persons
                .filter(
                  (pChild) =>
                    (pChild.biologicalMother?.equals(rel.person1Id) &&
                      pChild.biologicalFather?.equals(rel.person2Id)) ||
                    (pChild.biologicalMother?.equals(rel.person2Id) &&
                      pChild.biologicalFather?.equals(rel.person1Id)) ||
                    (pChild.legalParents.some((lp) =>
                      lp.parentId.equals(rel.person1Id)
                    ) &&
                      pChild.legalParents.some((lp) =>
                        lp.parentId.equals(rel.person2Id)
                      ))
                )
                .map((pChild) =>
                  mongoPersonIdToGedcomIndiId.get(pChild._id.toString())
                )
                .filter(Boolean);
              ...
              childrenOfThisCouple.forEach((childGedId) => {
                if (childGedId && !addedChildrenToFam.has(childGedId)) {
                  ...
                  personGedcomMapWithFamLinks[childGedId].famc.push(
                    gedcomFamId
                  );
          ...
          persons.forEach((person) => {
            const gedcomIndiId = mongoPersonIdToGedcomIndiId.get(
              person._id.toString()
            )!;
            ...
            const addTempLine = (
              ...
              if (value || typeof subLines === "function") {
                currentIndiLines.push(
                  `${level} ${tag}${value ? " " + value : ""}`
                );
            ...
            if (person.dateOfBirth || person.placeOfBirth) {
              addTempLine(1, "BIRT", undefined, () => {
                ...
                if (person.placeOfBirth)
                  addTempLine(2, "PLAC", person.placeOfBirth);
              });
            }
            if (
              person.dateOfDeath ||
              ...
              addTempLine(1, "DEAT", undefined, () => {
                ...
                if (person.placeOfDeath)
                  addTempLine(2, "PLAC", person.placeOfDeath);
                if (person.causeOfDeath)
                  addTempLine(2, "CAUS", person.causeOfDeath);
              });
            }
            person.identifiers?.forEach((id) => {
              if (id.type === "Email") addTempLine(1, "EMAIL", id.value);
                ...
                addTempLine(1, `_IDNO ${id.value}`, undefined, () => {
                  if (id.type) addTempLine(2, "TYPE", id.type);
                  if (id.verificationStatus)
                    addTempLine(2, "STAT", id.verificationStatus);
            ...
            personGedcomMapWithFamLinks[gedcomIndiId].famc.forEach((famcId) =>
              addTempLine(1, "FAMC", famcId)
            );
            personGedcomMapWithFamLinks[gedcomIndiId].fams.forEach((famsId) =>
              addTempLine(1, "FAMS", famsId)
            );
          ...
          finalGedcomLines.push(
            ...gedcomLines.filter(
              (line) =>
                line.includes(" FAM") ||
                line.startsWith("1 HUSB") ||
                line.startsWith("1 WIFE") ||
                line.startsWith("1 CHIL") ||
                line.startsWith("1 MARR") ||
                line.startsWith("1 DIV") ||
                (line.startsWith("1 EVEN") && !line.includes("INDI")) ||
                (line.startsWith("1 NOTE") && !line.includes("INDI")) ||
                (line.startsWith("2 DATE") && !line.includes("INDI")) ||
                (line.startsWith("2 PLAC") && !line.includes("INDI"))
            )
          );
          ...
          span.setStatus({ code: SpanStatusCode.ERROR, message: err.message });
          span.end();
          logger.error("Error exporting GEDCOM:", {
            error: err.message,
            stack: err.stack,
            userId: req.user?.id,
            treeId: req.params.treeId,
