...
import SearchFilters, { FilterFormState } from '../components/search/SearchFilters'; // Import SearchFilters

// Dummy Loading component
const LoadingSpinner = ({ text = "Loading search results..." }: { text?: string }) => (
...
const Button = ({ children, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & {variant?: string}) => (
  <button
    className={`px-4 py-2 font-semibold rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2
                ${props.disabled ? 'text-gray-400 bg-gray-200 dark:bg-gray-700 dark:text-gray-500 cursor-not-allowed' :
                  (props.variant === 'outline' ? 'text-blue-600 border border-blue-600 hover:bg-blue-50 dark:text-blue-400 dark:border-blue-400 dark:hover:bg-gray-700' :
...
const SearchResultsPage: React.FC = () => {
  ...
  const parseTypes = (typeString: string | null): SearchableTypeParam[] => {
    return typeString ? typeString.split(',').map(t => t.trim()).filter(Boolean) as SearchableTypeParam[] : [];
  };
  ...
  const currentSearchParams = useMemo<SearchParams>(() => {
    ...
    const sortBy = (searchParams.get('sortBy') || 'relevance') as SortByType;
    const sortOrder = (searchParams.get('sortOrder') || 'desc') as SortOrderType;
  ...
  const { results, pagination, isLoading, error, fetchSearchResults } = useSearchResults(currentSearchParams);
  ...
  const handlePageChange = (newPage: number) => {
    if (newPage > 0 && newPage <= (pagination?.totalPages || 1) && newPage !== currentSearchParams.page) {
  ...
  const handleFilterSubmit = (formFilters: Partial<FilterFormState>) => {
    ...
    Object.entries(formFilters).forEach(([key, value]) => {
        if (key === 'tags' && typeof value === 'string') {
            ...
            (searchApiFilters as any)[key] = tagsArray.length > 0 ? tagsArray : undefined;
  ...
  const renderResultItem = (item: SearchResultItem) => {
    switch (item._index) {
      case 'persons': // Or whatever your person index name is
        return <PersonSearchResultItem key={item._id} person={item._source as PersonData} />;
      case 'events': // Or whatever your event index name is
        return <EventSearchResultItem key={item._id} event={item._source as EventData} />;
      case 'comments': // Or whatever your comment index name is
        return <CommentSearchResultItem key={item._id} comment={item._source as CommentData} />;
      default:
        return <div key={item._id} className="p-2 my-1 border rounded text-xs dark:text-gray-400">Unknown result type: {item._index} - {item._id}</div>;
    }
  };
    ...
    <div className="container mx-auto p-4 sm:p-6 lg:p-8 min-h-screen">
      <header className="mb-6 pb-4 border-b dark:border-gray-700">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 dark:text-white">
          Search Results
          {query && <span className="text-gray-600 dark:text-gray-400 text-xl sm:text-2xl"> for "{query}"</span>}
        </h1>
      </header>

      <div className="flex flex-col md:flex-row gap-6 lg:gap-8">
        <aside className="md:w-1/3 lg:w-1/4 xl:w-1/5 space-y-6"> {/* Sidebar for filters and sort */}
          <SearchFilters
          ...
          <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg shadow space-y-3 sticky top-6">
            <h3 className="text-md font-semibold text-gray-700 dark:text-gray-200">Sort Options</h3>
            <div>
                ...
                <Select
                    ...
                    onChange={(e) => handleSortChange(e.target.value as SortByType, undefined)}
                >
            ...
            <div>
                ...
                <Select
                    ...
                    onChange={(e) => handleSortChange(undefined, e.target.value as SortOrderType)}
                >
        ...
        <main className="md:w-2/3 lg:w-3/4 xl:w-4/5"> {/* Main content area for results */}
      ...
      {!isLoading && !error && results.length === 0 && !query && (
         <div className="text-center py-10">
            <p className="text-lg text-gray-500 dark:text-gray-400">Enter a search term above to begin.</p>
        </div>
      )}
            ...
            items.length > 0 && (
              <section key={type}>
                <h2 className="text-xl font-semibold text-gray-700 dark:text-gray-300 mb-3 border-b pb-2">
                  {typeToTitleMapping[type] || `Results from ${type}`} ({items.length})
                </h2>
