...
} from "./auth"; // Assuming auth.ts exports necessary types
...
export type FrontendRegisterPayload = Omit<
  ApiRegisterData,
  "preferredLanguage"
>;
...
export const authService = {
  login: async (
    payload: FrontendLoginPayload
  ): Promise<FrontendAuthResponse> => {
    ...
    // Since backend might not return user data in login response, handle this case
    let userData: UserData;
    if (response.user) {
      userData = mapApiUserToUserData(response.user);
    } else {
      // Placeholder user data if not returned, or fetch separately if needed
      userData = {
        id: "",
        email: payload.email,
        firstName: "",
  ...
  register: async (
    payload: FrontendRegisterPayload
  ): Promise<FrontendAuthResponse> => {
  ...
  resetPassword: async (payload: {
    token: string;
    newPassword: string;
    email?: string /* if backend needs it */;
  ...
  verifyMfa: async (code: string): Promise<FrontendAuthResponse> => {
    ...
    const response: ApiLoginResponse = await authApi.login({
      email: "",
      password: "",
      mfaCode: code,
