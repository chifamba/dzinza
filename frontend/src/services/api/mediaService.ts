...
class MediaService {
  private baseURL = "/api/files"; // Updated for direct nginx routing to storage-service
  ...
  async uploadMedia(formData: FormData): Promise<ApiFile> {
    ...
    const response: AxiosResponse<{ data: ApiFile }> = await apiClient.post(
      // Assuming response is { data: ApiFile } like other services
      `${this.baseURL}/upload`,
      formData,
      {
        headers: {
  ...
  async getMediaDetails(mediaId: string): Promise<ApiFile> {
    const response: AxiosResponse<{ data: ApiFile }> = await apiClient.get(
  ...
  }): Promise<{ data: ApiFile[]; meta: PaginationInfo }> {
    // Define a proper meta type
    const response: AxiosResponse<{ data: ApiFile[]; meta: PaginationInfo }> =
