...
import { FamilyTree, FamilyMember, Relationship, Event } from '../../types/genealogy'; // Adjust path as needed
...
class GenealogyService {
  ...
  async getFamilyTree(treeId: string): Promise<FamilyTree> {
    const response: AxiosResponse<FamilyTree> = await apiClient.get(`${this.treeBaseURL}/${treeId}`);
    return response.data;
  }

  async createFamilyTree(data: Pick<FamilyTree, 'name' | 'description' | 'privacy'>): Promise<FamilyTree> {
    const response: AxiosResponse<FamilyTree> = await apiClient.post(this.treeBaseURL, data);
    return response.data;
  }
  ...
  async updateFamilyTree(treeId: string, data: Partial<Pick<FamilyTree, 'name' | 'description' | 'privacy' | 'settings'>>): Promise<FamilyTree> {
    const response: AxiosResponse<FamilyTree> = await apiClient.put(`${this.treeBaseURL}/${treeId}`, data);
    return response.data;
  }
  ...
  async getPersons(familyTreeId: string, search?: string, page?: number, limit?: number): Promise<PaginatedPersonsResponse> {
    ...
    const response: AxiosResponse<PaginatedPersonsResponse> = await apiClient.get(this.personBaseURL, { params });
    return response.data;
  }

  async getPersonDetails(personId: string, treeId?: string): Promise<FamilyMember> {
    ...
    const response: AxiosResponse<FamilyMember> = await apiClient.get(`${this.personBaseURL}/${personId}${treeId ? `?familyTreeId=${treeId}` : ''}`);
    return response.data;
  }

  async addPersonToTree(familyTreeId: string, personData: Omit<FamilyMember, 'id' | 'parentIds' | 'childIds' | 'spouseIds'>): Promise<FamilyMember> {
    // Endpoint might be /api/family-trees/:familyTreeId/persons or just /api/persons with familyTreeId in payload
    const response: AxiosResponse<FamilyMember> = await apiClient.post(`${this.personBaseURL}`, { ...personData, familyTreeId });
    return response.data;
  }

  async updatePersonInTree(personId: string, personData: Partial<FamilyMember>): Promise<FamilyMember> {
     // familyTreeId might be in personData or not needed if personId is globally unique
    const response: AxiosResponse<FamilyMember> = await apiClient.put(`${this.personBaseURL}/${personId}`, personData);
    return response.data;
  }

  async deletePersonFromTree(personId: string, familyTreeId?: string): Promise<void> {
    // familyTreeId might be passed as query param or in body if needed by backend for auth/scoping
    await apiClient.delete(`${this.personBaseURL}/${personId}${familyTreeId ? `?familyTreeId=${familyTreeId}` : ''}`);
  }

  // Relationship Methods
  async addRelationship(relationshipData: Omit<Relationship, 'id'>): Promise<Relationship> {
    const response: AxiosResponse<Relationship> = await apiClient.post(this.relationshipBaseURL, relationshipData);
    return response.data;
  }
  ...
  async getEventsForPerson(personId: string, familyTreeId?: string, page?: number, limit?: number): Promise<PaginatedEventsResponse> {
    ...
    const response: AxiosResponse<PaginatedEventsResponse> = await apiClient.get(this.eventsBaseURL, { params });
    return response.data;
  }

  async addEvent(eventData: Omit<Event, 'id'>): Promise<Event> {
    const response: AxiosResponse<Event> = await apiClient.post(this.eventsBaseURL, eventData);
    return response.data;
  }

  async updateEvent(eventId: string, eventData: Partial<Omit<Event, 'id'>>): Promise<Event> {
    const response: AxiosResponse<Event> = await apiClient.put(`${this.eventsBaseURL}/${eventId}`, eventData);
    return response.data;
  }
