...
const EventSearchResultItem: React.FC<EventSearchResultItemProps> = ({
  item,
}) => {
  ...
  const titleContent = item.highlight?.title?.[0] ? (
    <span
      dangerouslySetInnerHTML={createSanitizedHTML(item.highlight.title[0])}
    />
  ) : (
  ...
  const contentSnippet = item.highlight?.plainTextContent?.[0] ? (
    <span
      dangerouslySetInnerHTML={createSanitizedHTML(
        item.highlight.plainTextContent[0]
      )}
    />
  ) : event.plainTextContent ? (
    event.plainTextContent.substring(0, 200) +
    (event.plainTextContent.length > 200 ? "..." : "")
  ) : (
  ...
  const categoryContent = item.highlight?.category?.[0] ? (
    <span
      dangerouslySetInnerHTML={createSanitizedHTML(item.highlight.category[0])}
    />
  ) : (
    event.category
  );
  ...
  return (
    <div className="p-4 my-2 bg-white dark:bg-gray-800 shadow rounded-lg hover:shadow-md transition-shadow">
      <Link to={`/events/${item._id}`} className="group">
        {" "}
        {/* Use item._id for link */}
      ...
      {(event.tags && event.tags.length > 0) || item.highlight?.tags ? (
        <div className="flex flex-wrap gap-1.5">
          <Hash
            size={14}
            className="inline mr-1 opacity-70 mt-0.5 text-gray-500 dark:text-gray-400"
          />
          {/* If tags are highlighted, they come as an array of highlighted strings */}
          {item.highlight?.tags
            ? item.highlight.tags.map((tagHtml, index) => (
                <span
                  key={`highlight-tag-${index}`}
                  className="px-2 py-0.5 bg-blue-100 dark:bg-blue-800 text-blue-700 dark:text-blue-200 rounded-full text-xs"
