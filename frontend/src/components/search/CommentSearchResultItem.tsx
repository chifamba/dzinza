...
const CommentSearchResultItem: React.FC<CommentSearchResultItemProps> = ({
  item,
}) => {
  ...
  const contentDisplay = item.highlight?.content?.[0] ? (
    <span
      dangerouslySetInnerHTML={createSanitizedHTML(item.highlight.content[0])}
    />
  ) : comment.content ? (
  ...
  const userNameDisplay = item.highlight?.userName?.[0] ? (
    <span
      dangerouslySetInnerHTML={createSanitizedHTML(item.highlight.userName[0])}
    />
  ) : (
  ...
  if (comment.resourceType && comment.resourceId) {
    const resourcePath =
      comment.resourceType.toLowerCase() === "event"
        ? "events"
        : comment.resourceType.toLowerCase() === "story"
        ? "stories"
        : "#";
    if (resourcePath !== "#") {
    ...
    <div className="p-4 my-2 bg-white dark:bg-gray-800 shadow rounded-lg hover:shadow-md transition-shadow">
      <Link to={resourceLink} className="group">
        <div className="flex items-start space-x-2 mb-1">
          <MessageSquare
            size={18}
            className="inline mt-0.5 text-yellow-600 dark:text-yellow-400 opacity-80"
          />
