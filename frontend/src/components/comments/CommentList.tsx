...
import {
  ...
  CommentsApiResponse,
  CommentResourceType,
} from "../../types/comments"; // Adjust path
...
const CommentList: React.FC<CommentListProps> = ({
  ...
  const [editingComment, setEditingComment] = useState<CommentData | null>(
    null
  ); // For editing a specific comment

  const fetchComments = useCallback(
    async (pageToFetch = 1, newLimit?: number) => {
      setIsLoading(true);
      setError(null);
      const currentLimit = newLimit || pagination.limit;
      try {
        const response = await fetch(
          `/api/${resourceType}/${resourceId}/comments?page=${pageToFetch}&limit=${currentLimit}&sort=createdAt`
        );
        if (!response.ok) {
        ...
        logger.error(
          `Error fetching comments for ${resourceType} ${resourceId}:`,
          err
        );
      } finally {
        setIsLoading(false);
  ...
  const handleCommentAdded = (newComment: CommentData) => {
    if (editingComment && newComment._id === editingComment._id) {
      // If it was an edit
      setComments((prevComments) =>
        prevComments.map((c) => (c._id === newComment._id ? newComment : c))
      );
  ...
  const renderCommentNode = (comment: CommentData, depth: number = 0) => (
    <div
      key={comment._id}
      className={`${depth > 0 ? "ml-4 sm:ml-8 md:ml-12" : ""} py-1`}
    >
      <CommentItem
        comment={comment}
  ...
  return (
    <div className="mt-6 space-y-4">
      ...
      {editingComment &&
        !replyingTo && ( // General message if an edit form is open elsewhere
          <p className="text-sm text-blue-600">Editing a comment...</p>
        )}
      {replyingTo &&
        !editingComment && ( // General message if a reply form is open elsewhere
