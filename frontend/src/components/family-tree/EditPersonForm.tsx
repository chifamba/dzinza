...
import ProfileAvatar from "../ProfileAvatar"; // Import ProfileAvatar for realistic photos
    ...
    return (
      <div className="space-y-3 p-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 text-sm">
          ...
          {birthDate && (
            <div>
              <strong className="font-medium text-gray-600">Birth Date:</strong>{" "}
              {birthDate}
            </div>
          )}
          {deathDate && (
            <div>
              <strong className="font-medium text-gray-600">Death Date:</strong>{" "}
              {deathDate}
            </div>
          )}
        </div>
          ...
          {person.parentIds && person.parentIds.length > 0 && (
            <div className="mb-2">
              ...
              <ul className="list-disc list-inside text-sm text-gray-500 ml-4">
                {person.parentIds.map((id) => (
                  <li key={id}>{getNameById(id)}</li>
                ))}
              </ul>
            </div>
          )}
