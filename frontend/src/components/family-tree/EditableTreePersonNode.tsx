...
interface EditableTreePersonNodeProps {
  ...
  onAddPerson: (relativeToId: string, relationType: 'parent' | 'child' | 'spouse') => void;
...
const EditableTreePersonNode: React.FC<EditableTreePersonNodeProps> = ({
  ...
  const isSelectedForRelationship = selectedPersonForRelationship?.id === currentPersonId;
  ...
  const handleRelationshipClick = () => {
    if (isRelationshipCreationMode && selectedPersonForRelationship && selectedPersonForRelationship.id !== currentPersonId) {
      ...
      {isAuthenticated && isEditMode && isHovered && (
        <>
          ...
          <button
            ...
            title={isRelationshipCreationMode && selectedPersonForRelationship ? "Click another person to create relationship" : "Start creating relationship"}
          >
            <Link size={14} />
          </button>
        </>
      ...
      >
        <div className="p-1.5 sm:p-2 md:p-3">
          ...
          <div className="mt-1.5 text-center space-x-0.5 sm:space-x-1 flex flex-wrap justify-center gap-0.5 sm:gap-1">
            <Button 
              onClick={(e) => { e.stopPropagation(); onViewProfile(memberForHandlers); }} 
              variant="ghost" 
            ...
            {isAuthenticated && (
              <>
                <Button 
                  onClick={(e) => { e.stopPropagation(); onEdit(memberForHandlers); }} 
                  variant="secondary" 
                  className="text-[0.6rem] px-1 py-0.5 sm:text-xs sm:px-1.5 sm:py-1 md:px-2 md:py-1 flex items-center gap-1"
                >
                  <Edit3 size={10} />
                ...
                {isEditMode ? (                <Button 
                  onClick={(e) => { e.stopPropagation(); onDelete(memberForHandlers); }} 
                  variant="destructive" 
                  className="text-[0.6rem] px-1 py-0.5 sm:text-xs sm:px-1.5 sm:py-1 md:px-2 md:py-1 flex items-center gap-1"
                >
                  <Trash2 size={10} />
                  ...
                  <Button 
                    onClick={(e) => { e.stopPropagation(); onConnectRelationship(memberForHandlers); }} 
                    variant="ghost" 
                    className="text-[0.6rem] px-1 py-0.5 sm:text-xs sm:px-1.5 sm:py-1 md:px-2 md:py-1 flex items-center gap-1"
                  >
                    <UserPlus size={10} />
          ...
          <div className="mt-1.5 pt-1 border-t">
            ...
            {/* Children */}
            {memberForHandlers.childIds && memberForHandlers.childIds.length > 0 && (
              <div>
                <h4 className="text-[0.7rem] sm:text-xs font-semibold">Children:</h4>
                {memberForHandlers.childIds.map(childId => (
                  <div key={childId} className="flex justify-between items-center text-[0.6rem] sm:text-[0.65rem] md:text-xs">
                    <span className="truncate" title={getNameById(childId)}>{getNameById(childId)}</span>
                    {isAuthenticated && isEditMode && (
                      <Button
                        variant="destructive" 
                        className="text-[0.5rem] px-0.5 sm:text-[0.55rem] sm:px-1 md:text-xs"
                        onClick={(e) => { e.stopPropagation(); onDeleteRelationship(currentPersonId, childId, 'PARENT_CHILD_PARENT_PERSPECTIVE'); }}
                      >
                        X
                      </Button>
                    )}
