      ...
      <div className="p-2 sm:p-3">
        ...
        {memberForHandlers.profileImageUrl && (
          <img
            ...
            alt={personName}
            className="w-12 h-12 sm:w-16 sm:h-16 rounded-full mx-auto my-1 object-cover border"
          />
        )}
        <div className="text-[0.65rem] sm:text-xs text-gray-600 mb-1 text-center">
        ...
        <div className="mt-1.5 text-center space-x-0.5 sm:space-x-1 flex flex-wrap justify-center gap-0.5 sm:gap-1">
          <Button onClick={(e) => { e.stopPropagation(); onViewProfile(memberForHandlers); }} variant="info" size="xxs" className="px-1.5 py-0.5 sm:px-2 sm:py-1">View</Button>
          <Button onClick={(e) => { e.stopPropagation(); onEdit(memberForHandlers); }} variant="secondary" size="xxs" className="px-1.5 py-0.5 sm:px-2 sm:py-1">Edit</Button>
          <Button onClick={(e) => { e.stopPropagation(); onConnectRelationship(memberForHandlers); }} variant="ghost" size="xxs" className="px-1.5 py-0.5 sm:px-2 sm:py-1">Connect</Button>
          <Button onClick={(e) => { e.stopPropagation(); onDelete(memberForHandlers); }} variant="danger" size="xxs" className="px-1.5 py-0.5 sm:px-2 sm:py-1">Delete</Button>
        </div>
          ...
          {memberForHandlers.parentIds && memberForHandlers.parentIds.length > 0 && (
            <div className="mb-0.5">
              ...
              {memberForHandlers.parentIds.map(parentId => (
                <div key={parentId} className="flex justify-between items-center text-[0.65rem] sm:text-xs">
                  <span className="truncate" title={getNameById(parentId)}>{getNameById(parentId)}</span>
                  <Button
                    variant="outlineDanger" size="xxs" className="px-1 py-0.5 text-[0.55rem] sm:text-xs"
          ...
          {memberForHandlers.spouseIds && memberForHandlers.spouseIds.length > 0 && (
            <div className="mb-0.5">
              ...
              {memberForHandlers.spouseIds.map(spouseId => (
                <div key={spouseId} className="flex justify-between items-center text-[0.65rem] sm:text-xs">
        ...
        {nodeDatum.children && nodeDatum.children.length > 0 && (
          <p className="text-[0.65rem] sm:text-xs text-center mt-1 text-gray-500 italic">
