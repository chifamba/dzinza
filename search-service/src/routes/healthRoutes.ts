...
router.get("/health", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (esClient) {
      ...
      if (
        pingResponse === true ||
        (typeof pingResponse === "object" &&
          (pingResponse as any).body === true)
      ) {
        // Compatibility for older client versions
        esStatus = "UP";
        esReason = "";
      } else {
      ...
      esReason =
        "Elasticsearch client is not available (initialization may have failed).";
    }
  } catch (error: any) {
    logger.error(
      `${SERVICE_NAME} /health - Elasticsearch check failed:`,
      error
    );
    esStatus = "DOWN";
    if (!clientAvailable) {
      ...
      esReason =
        error.message || "Elasticsearch ping failed or client unavailable.";
    }
  }
