...
router.post(
  ...
  [
    body("type")
      .isIn(validIndexTypes)
      .withMessage(`Type must be one of: ${validIndexTypes.join(", ")}`),
    body("documentId")
      .isString()
      .notEmpty()
    ...
    body("document")
      .isObject()
      .withMessage("document body is required and must be an object."),
  ...
  async (req: Request, res: Response) => {
    ...
    if (!type || !documentId || !document) {
      return res.status(400).json({
        errors: [{ msg: "Type, documentId, and document are required" }],
      });
    }

    try {
      const indexName = typeToIndexName(type as SearchableType);
      ...
      if (
        ...
        typeof documentToIndex.content === "string"
      ) {
        documentToIndex.plainTextContent = serviceInstance.stripHtml(
          documentToIndex.content
        );
      ...
      if (type === "person") {
        ...
        if (documentToIndex.lastName)
          fullName += (fullName ? " " : "") + documentToIndex.lastName;
      ...
      res.status(200).json({
        message: `Document ${documentId} of type ${type} indexed successfully.`,
      });
    } catch (error: any) {
      logger.error(
        `Error processing /api/index request for type ${type}, ID ${documentId}:`,
        error
      );
      ...
      res
        .status(500)
        .json({ message: "Failed to index document.", error: error.message });
    }
  }
);
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!type || !id) {
      return res
        .status(400)
        .json({ errors: [{ msg: "Type and ID parameters are required" }] });
    }
    ...
    try {
      ...
      res.status(200).json({
        message: `Document ${id} of type ${type} deleted successfully from index.`,
      });
      // Or res.status(204).send();
    } catch (error: any) {
      logger.error(
        `Error processing /api/index/:type/:id DELETE request for type ${type}, ID ${id}:`,
        error
      );
      if (error.message.startsWith("Invalid type specified")) {
        return res.status(400).json({ message: error.message });
      ...
      if (
        ...
        error.message.includes("for deletion")
      ) {
        return res.status(200).json({
          message: `Document ${id} of type ${type} was not found in index, considered deleted.`,
        });
      }
      res.status(500).json({
...
router.get(
  ...
  [
    ...
    query("type")
      ...
      .custom((value: string) => {
        ...
        const invalidTypes = types.filter(
          (t) => !validIndexTypes.includes(t as SearchableType)
        );
        if (invalidTypes.length > 0) {
          throw new Error(
            `Invalid search type(s): ${invalidTypes.join(
              ", "
            )}. Allowed types: ${validIndexTypes.join(", ")}`
          );
        }
        return true;
      }),
    ...
    query("sortBy")
      .optional()
      .isIn(validSortBy)
      .withMessage(`SortBy must be one of: ${validSortBy.join(", ")}`)
      .default("relevance"),
    query("sortOrder")
      .optional()
      .isIn(validSortOrder)
      .withMessage(`SortOrder must be one of: ${validSortOrder.join(", ")}`)
      .default("desc"),
    // New filter validations
    query("eventDateFrom")
      .optional()
      .isISO8601()
      .withMessage("eventDateFrom must be a valid ISO8601 date."),
    query("eventDateTo")
      .optional()
      .isISO8601()
      .withMessage("eventDateTo must be a valid ISO8601 date."),
    query("birthDateFrom")
      .optional()
      .isISO8601()
      .withMessage("birthDateFrom must be a valid ISO8601 date."),
    query("birthDateTo")
      .optional()
      .isISO8601()
      .withMessage("birthDateTo must be a valid ISO8601 date."),
    query("deathDateFrom")
      .optional()
      .isISO8601()
      .withMessage("deathDateFrom must be a valid ISO8601 date."),
    query("deathDateTo")
      .optional()
      .isISO8601()
      .withMessage("deathDateTo must be a valid ISO8601 date."),
  ...
  async (req: Request, res: Response) => {
    ...
    if (type) {
      searchParams.types = type
        .split(",")
        .map((t: string) => t.trim()) as SearchableType[];
    }
    if (tags) {
    ...
    try {
      ...
      logger.error(
        `Error processing /api/search request for query "${q}":`,
        error
      );
      res
        .status(500)
