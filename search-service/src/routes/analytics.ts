    ...
    try {
      ...
      const formattedData = popularQueries
        ...
        .map((bucket: any) => ({
          ...
          percentage:
            totalSearches > 0 ? (bucket.doc_count / totalSearches) * 100 : 0,
        }));
 ...
 *         description: Internal server error
 */
router.get("/search-performance", async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user?.id;
    ...
    const totalSearches = searchTypes.reduce(
      (sum: number, bucket: any) => sum + bucket.doc_count,
      0
    );
    ...
    res.json({
      ...
      data: {
        ...
        performance: {
          ...
          searchesWithResults:
            Math.round(
              ((avgResults > 0 ? totalSearches : 0) * 100) / totalSearches
            ) || 0,
        },
      },
      meta: {
