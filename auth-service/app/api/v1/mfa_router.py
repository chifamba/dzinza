...
async def setup_mfa(
    ...
    await user_crud.update_user(db, user=current_user, user_in={"mfa_secret": otp_secret, "mfa_enabled": False})
...
async def verify_mfa_setup_and_enable(
    ...
    await user_crud.set_mfa_secret(db, user=current_user, secret=current_user.mfa_secret, enabled=True)
...
async def disable_mfa(
    ...
    current_user: User = Depends(get_current_active_user), # User must be logged in
    db: AsyncSession = Depends(get_db_session)
):
...
async def login_verify_otp(
    ...
    partially_authenticated_user: User = Depends(get_current_active_user), # Needs refinement
    db: AsyncSession = Depends(get_db_session),
