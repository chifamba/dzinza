...
func main() {
	logger := logging.NewLogger("auth_service")
	...
	// Auto Migrate
	if err := db.AutoMigrate(&models.User{}, &models.Role{}, &models.UserTreeRole{}); err != nil {
	...
	r.GET("/health", health.HealthCheckHandler("auth_service"))

	handlers.RegisterRoutes(r, handler, cfg, rdb)

	port := cfg.AuthServicePort
	if port == 0 {
		port = 8003 // Default as per spec
	}

	addr := fmt.Sprintf(":%d", port)
	logger.Info("starting auth_service", slog.String("addr", addr))
